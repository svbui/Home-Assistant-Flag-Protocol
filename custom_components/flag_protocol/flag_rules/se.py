from datetime import datetime, timedelta, time

def calculate_easter(year: int) -> datetime:
    """Compute Easter Sunday for a given year (Anonymous Gregorian algorithm)."""
    a = year % 19
    b = year // 100
    c = year % 100
    d = b // 4
    e = b % 4
    f = (b + 8) // 25
    g = (b - f + 1) // 3
    h = (19 * a + b - d - g + 15) % 30
    i = c // 4
    k = c % 4
    l = (32 + 2 * e + 2 * i - h - k) % 7
    m = (a + 11 * h + 22 * l) // 451
    month = (h + l - 7 * m + 114) // 31
    day = ((h + l - 7 * m + 114) % 31) + 1
    return datetime(year, month, day)

def get_flag_status(now: datetime) -> tuple[str, str]:
    """Return (flag_type, reason) for Sweden for this datetime."""
    year = now.year
    easter = calculate_easter(year)
    whit_sunday = easter + timedelta(days=49)
    # Midsummer Day: Saturday between June 20 and 26
    midsummer = next(
        datetime(year, 6, d)
        for d in range(20, 27)
        if datetime(year, 6, d).weekday() == 5
    )

    fixed_days = [
        (1, 1,    "New Year's Day"),
        (1, 28,   "Name day of HM The King"),
        (3, 12,   "Name day of HRH The Crown Princess"),
        (4, 30,   "Birthday of HM The King"),
        (5, 1,    "First of May"),
        (5, 29,   "Veterans Day"),
        (6, 6,    "National Day of Sweden"),
        (7, 14,   "Birthday of HRH The Crown Princess"),
        (8, 8,    "Name day of HM The Queen"),
        (10, 24,  "United Nations Day"),
        (11, 6,   "Commemoration of King Gustav II Adolf"),
        (12, 10,  "Nobel Day"),
        (12, 23,  "Birthday of HM The Queen"),
        (12, 25,  "Christmas Day")
    ]

    # Determine today's reason if any
    if (now.month, now.day) in {(m, d) for m, d, _ in fixed_days}:
        reason = next(r for m, d, r in fixed_days if m == now.month and d == now.day)
    elif now.date() == easter.date():
        reason = "Easter Sunday"
    elif now.date() == whit_sunday.date():
        reason = "Whit Sunday"
    elif now.date() == midsummer.date():
        reason = "Midsummer Day"
    else:
        return "no_flag", "No flag today"

    # Time‐based visibility rules
    # March–Oct: start at 08:00; Nov–Feb: start at 09:00
    start_time = time(9) if now.month in (11, 12, 1, 2) else time(8)
    end_time = time(21, 0)
    current = now.time()

    if not (start_time <= current <= end_time):
        return "no_flag", f"Flag allowed only between {start_time.strftime('%H:%M')} and 21:00"

    return "full_mast", reason

def get_next_flag_day(now: datetime) -> tuple[int, str, str]:
    """Return (days_until, reason, flag_type) for next Swedish flag day."""
    for i in range(1, 366):
        future = now + timedelta(days=i)
        flag_type, reason = get_flag_status(future)
        if flag_type != "no_flag":
            return i, reason, flag_type
    return -1, "Unknown", "no_flag"
